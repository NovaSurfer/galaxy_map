...
pub fn generate_galaxy_vectors(size: u32, arm_separation_dist: f32, arm_offset_max: f32, rotation_factor: f32, random_offset_xy: f32) -> Vec<f32>
{
    let mut rnd = Random::default();
    (0..size).into_iter().flat_map(|_| {
        let mut dist: f32 = rnd.gen_range(0.0..50.0);
        dist = dist.powf(2.0);

        let mut angle = rnd.gen_range(0.0..50.0) * 2.0 * std::f32::consts::PI;

        let mut arm_offset = rnd.gen_range(0.0..50.0) * arm_offset_max;
        ...
        let rnd_offset_x: f32 = rnd.gen_range(0.0..50.0) * random_offset_xy;
        let rnd_offset_y: f32 = rnd.gen_range(0.0..50.0) * random_offset_xy;
