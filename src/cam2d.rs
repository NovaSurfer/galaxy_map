...
impl Camera2d
{
    pub fn new(aspect_ratio: f32, speed: f32, zoom: f32) -> Self {
        let ortho = Mat4::orthographic_rh_gl(-aspect_ratio * zoom, aspect_ratio * zoom, -zoom, zoom, -1.0, 1.0);
        Self {
            projection: ortho,
            view: Mat4::IDENTITY,
    ...
    {
        self.view = Mat4::from_translation(vec3(-self.position.x, -self.position.y, 0.0))
            * Mat4::from_scale(vec3(self.zoom, self.zoom, 1.0))
            * Mat4::from_translation(vec3(400.0, 300.0, 0.0));
        self.view_projection = self.projection * self.view;
    }
}